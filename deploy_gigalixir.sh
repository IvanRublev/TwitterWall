...
gigalixir config:set -a $app COOKIE=`mix phx.gen.secret`
gigalixir config:set -a $app LV_SIGNING_SALT=`mix phx.gen.secret | tail -c 8`
...
echo "yarn_version=${yarn}" >> $npm_bp_cfg
